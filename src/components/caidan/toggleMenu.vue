<template>
  <div>
    ...
    <div class="menu_item" v-for="(item,index) in menuItems" :id="item.name" @click="clickMenu(item,index)" :key="index">
      <img :src="item.src">
    </div>
	<div class="bg" v-if="openFlag"></div>
  </div>
</template>
  ...
  export default {
    ...
    mounted() {
      ...
      this.menuItems.forEach((item) => {
        ...
        el.style.width = `${this.width * 0.5}px`;
        el.style.height = `${this.width * 0.5}px`;
